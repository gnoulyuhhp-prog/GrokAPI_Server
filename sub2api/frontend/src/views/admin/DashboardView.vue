        ...
        <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <div class="card p-4">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400">
                  Token Available (Retail)
                </p>
                <p class="text-xl font-bold text-emerald-600 dark:text-emerald-400">
                  {{ formatTokens(grokSalesCapacity) }}
                </p>
                <p class="text-xs text-green-600 dark:text-green-400">
                  {{ formatNumber(grokSalesCapacity) }} planning tokens · 50K × {{ formatNumber(grokActiveAccounts) }} active accounts
                </p>
              </div>
            </div>
          </div>
...
const GROK_SALES_TOKENS_PER_ACCOUNT = 50_000
const grokSalesCapacity = ref(0)
const grokActiveAccounts = ref(0)
...
// Retail planning capacity is intentionally separate from upstream xAI quota.
// Fetch only the active-account count; do not page through every credential on
// each dashboard load. Sold/reserved/used capacity needs the Vortex ledger.
const loadGrokSalesCapacity = async () => {
  try {
    const response = await adminAPI.accounts.list(1, 1, {
      platform: 'grok',
      status: 'active',
      lite: 'true'
    })
    const active = Math.max(0, Number(response.total) || 0)
    grokActiveAccounts.value = active
    grokSalesCapacity.value = active * GROK_SALES_TOKENS_PER_ACCOUNT
  } catch (error) {
    grokSalesCapacity.value = 0
    grokActiveAccounts.value = 0
    console.error('Error loading Grok retail capacity:', error)
  }
}
