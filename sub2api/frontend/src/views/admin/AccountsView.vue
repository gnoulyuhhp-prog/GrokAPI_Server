...
const USAGE_BATCH_CACHE_TTL = 15 * 60 * 1000
...
const reload = async () => {
  ...
  await baseReload()
  refreshTodayStatsBatch().catch((error) => {
    console.error('Failed to refresh account today stats:', error)
  })
}
...
const refreshAccountsIncrementally = async () => {
  ...
  try {
    ...
    refreshTodayStatsBatch().catch((error) => {
      console.error('Failed to refresh account today stats during auto refresh:', error)
    })
  } catch (error) {
