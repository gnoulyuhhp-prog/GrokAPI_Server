      ...
      <form id="key-form" @submit.prevent="handleSubmit" class="space-y-5">
        <div>
          ...
          <input
            ...
            type="text"
            class="input"
            placeholder="Để trống sẽ tự đặt: Grok_500k_01..."
        ...
        <div class="space-y-3">
          ...
          <div class="space-y-3">
            ...
            <div class="flex flex-wrap gap-2">
              <button
                v-for="pkg in [
                  ...
                  { label: '3M Token', tokens: 3000000 },
                  { label: '5M Token', tokens: 5000000 },
                  { label: '10M Token', tokens: 10000000 },
                  { label: '20M Token', tokens: 20000000 },
                  { label: '30M Token', tokens: 30000000 },
                  { label: '40M Token', tokens: 40000000 },
                  { label: '50M Token', tokens: 50000000 },
                  { label: '100M Token', tokens: 100000000 },
                  { label: '150M Token', tokens: 150000000 },
                ]"
                :key="pkg.tokens"
                type="button"
            ...
            <div>
              <label class="input-label text-xs">Số Token quy đổi (500K–150M, bước 500K)</label>
              <div class="relative">
                <input
              ...
              <p class="input-hint">
                Quota provider: <strong>${{ (formData.quota || 0).toFixed(4) }} USD</strong>
                <span class="mx-1">·</span>
                Giá bán tham khảo: <strong>{{ retailPriceLabel }}</strong>
                <span class="block">500K = 15.000đ · 1M = 29.000đ · từ 3M giảm 15%</span>
              </p>
            </div>
...
const formData = ref({
  ...
  token_amount: 500000 as number | null,
  enable_quota: false,
  quota: 1 as number | null,
  // Rate limit settings
...
const retailPriceVnd = computed(() => {
  const tokens = Number(formData.value.token_amount || 0)
  if (tokens < 500_000 || tokens > 150_000_000 || tokens % 500_000 !== 0) return null
  const basePrice = tokens === 500_000 ? 15_000 : (tokens / 500_000) * 14_500
  return Math.round(tokens >= 3_000_000 ? basePrice * 0.85 : basePrice)
})

const retailPriceLabel = computed(() => {
  return retailPriceVnd.value == null ? 'Không hợp lệ' : `${retailPriceVnd.value.toLocaleString('vi-VN')}đ`
})
...
const handleSubmit = async () => {
  ...
  if (!showEditModal.value) {
    const tokens = Number(formData.value.token_amount || 0)
    if (tokens < 500_000 || tokens > 150_000_000 || tokens % 500_000 !== 0) {
      appStore.showError('Số token phải từ 500K đến 150M và theo bước 500K')
      return
    }
  }
