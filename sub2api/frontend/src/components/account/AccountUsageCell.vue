<template>
  <div ref="rootRef" v-if="showUsageWindows">
    ...
    <template v-else-if="account.platform === 'grok' && account.type === 'oauth'">
      ...
      <div v-else-if="usageInfo" class="space-y-1">
        <div
          v-if="grokTokenQuotaDetails"
          data-testid="grok-available-tokens"
          class="space-y-0.5 rounded bg-emerald-50 px-1.5 py-1 text-[10px] text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-200"
        >
          <div class="font-semibold">
            {{ t('admin.accounts.usageWindow.grokAvailableTokens') }}
            {{ grokTokenQuotaDetails.remainingCompact }}
            <span class="font-normal opacity-80">({{ grokTokenQuotaDetails.remainingExact }})</span>
          </div>
          <div v-if="grokTokenQuotaDetails.limitExact">
            {{ t('admin.accounts.usageWindow.grokTokenLimit') }} {{ grokTokenQuotaDetails.limitExact }}
            <span v-if="grokTokenQuotaDetails.usedExact">
              · {{ t('admin.accounts.usageWindow.grokTokenUsed') }} {{ grokTokenQuotaDetails.usedExact }}
            </span>
          </div>
          <div v-if="grokTokenQuotaDetails.resetLabel">
            {{ t('admin.accounts.usageWindow.grokTokenReset') }} {{ grokTokenQuotaDetails.resetLabel }}
          </div>
        </div>
...
const formatGrokAvailableTokens = (value: number): string => {
  if (!Number.isFinite(value) || value < 0) return ''
  if (value < 1_000_000) return Math.trunc(value).toString()
  const divisor = value < 1_000_000_000 ? 1_000_000 : 1_000_000_000
  const suffix = value < 1_000_000_000 ? 'M' : 'B'
  const scaled = value / divisor
  const compact = scaled.toFixed(3).replace(/\.?0+$/, '')
  return `${compact}${suffix}`
}
const formatGrokTokenExact = (value: number): string => {
  return Math.trunc(value).toLocaleString('en-US')
}
const grokTokenQuotaDetails = computed(() => {
  const quota = usageInfo.value?.grok_token_quota
  const remaining = quota?.remaining
  if (typeof remaining !== 'number' || !Number.isFinite(remaining) || remaining < 0) return null
  const limit = quota?.limit
  const hasLimit = typeof limit === 'number' && Number.isFinite(limit) && limit >= 0
  const used = hasLimit ? Math.max(0, limit - remaining) : null
  const resetSource = quota?.reset_at || (quota?.reset_unix ? new Date(quota.reset_unix * 1000).toISOString() : '')
  const resetDate = resetSource ? new Date(resetSource) : null
  const resetLabel = resetDate && !Number.isNaN(resetDate.getTime())
    ...
    : ''
  return {
    remainingCompact: formatGrokAvailableTokens(remaining),
    remainingExact: formatGrokTokenExact(remaining),
    limitExact: hasLimit ? formatGrokTokenExact(limit) : '',
    usedExact: used == null ? '' : formatGrokTokenExact(used),
    resetLabel
  }
})
