...
type APIKey struct {
	...
	// RetailSale is set only when durable API-key sales ledgers prove that the
	// key was fulfilled as a prepaid retail entitlement.
	RetailSale bool
...
// HasRemainingQuota reports whether this key carries a finite quota that can
// still fund requests. It is intentionally based on the key's own durable quota
// fields, not on bot/sales provenance, so admin/web-created quota keys remain
// usable without requiring a matching sales ledger row.
func (k *APIKey) HasRemainingQuota() bool {
	return k != nil && k.Quota > 0 && k.QuotaUsed < k.Quota
}

// BypassesUserBalance reports whether eligibility and usage billing should use
// the API key entitlement instead of the owning user's balance.
func (k *APIKey) BypassesUserBalance() bool {
	return k != nil && (k.RetailSale || k.HasRemainingQuota())
}
