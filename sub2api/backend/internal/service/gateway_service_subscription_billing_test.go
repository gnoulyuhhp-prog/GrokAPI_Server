...
func TestBuildUsageBillingCommand_SubscriptionAppliesRateMultiplier(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:           "retail sale charges key quota without user balance",
			totalCost:      1.0,
			actualCost:     2.0,
			isSubscription: false,
			apiKeyQuota:    10,
		...
		{
			name:           "web quota key charges key quota without user balance",
			totalCost:      1.0,
			actualCost:     2.0,
			isSubscription: false,
			apiKeyQuota:    10,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			p := &postUsageBillingParams{
				...
				User:               &User{ID: 1},
				APIKey:             &APIKey{ID: 2, GroupID: &groupID, Quota: tt.apiKeyQuota, RetailSale: tt.retailSale},
			...
			if cmd.APIKeyQuotaCost != tt.wantKeyQuota {
				t.Errorf("APIKeyQuotaCost = %v, want %v", cmd.APIKeyQuotaCost, tt.wantKeyQuota)
			}
		})
	}
}
