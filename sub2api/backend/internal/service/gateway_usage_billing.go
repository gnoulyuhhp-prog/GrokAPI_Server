...
func postUsageBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps) {
	...
	if p.IsSubscriptionBill {
		...
		if cost.ActualCost > 0 && !p.APIKey.BypassesUserBalance() {
	...
	if !p.IsSubscriptionBill && p.Platform != "" && cost.ActualCost > 0 && p.User != nil && !p.APIKey.BypassesUserBalance() && deps.userPlatformQuotaRepo != nil {
...
func buildUsageBillingCommand(requestID string, usageLog *UsageLog, p *postUsageBillingParams) *UsageBillingCommand {
	...
	} else if p.Cost.ActualCost > 0 && !p.APIKey.BypassesUserBalance() {
...
func finalizePostUsageBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	} else if p.Cost.ActualCost > 0 && p.User != nil && !p.APIKey.BypassesUserBalance() {
	...
	if !p.IsSubscriptionBill && p.Platform != "" && p.Cost.ActualCost > 0 && p.User != nil && !p.APIKey.BypassesUserBalance() && deps.userPlatformQuotaRepo != nil {
...
func notifyBalanceLow(p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	if p.IsSubscriptionBill || p.APIKey.BypassesUserBalance() || p.Cost.ActualCost <= 0 || p.User == nil || deps.balanceNotifyService == nil {
		slog.Debug("notifyBalanceLow: skipped",
			"is_subscription", p.IsSubscriptionBill,
			"api_key_balance_exempt", p.APIKey.BypassesUserBalance(),
