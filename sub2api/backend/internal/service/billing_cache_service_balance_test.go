...
func TestCheckBillingEligibility_RetailSaleBypassesZeroBalance(t *testing.T) {
	cache := &balanceEligibilityCacheStub{balance: 0}
	cfg := &config.Config{}
	svc := NewBillingCacheService(cache, nil, nil, nil, nil, nil, cfg, nil)
	t.Cleanup(svc.Stop)

	err := svc.CheckBillingEligibility(context.Background(), &User{ID: 1}, &APIKey{ID: 2, RetailSale: true}, nil, nil, "")
	require.NoError(t, err)
}

func TestCheckBillingEligibility_WebQuotaKeyBypassesZeroBalance(t *testing.T) {
	cache := &balanceEligibilityCacheStub{balance: 0}
	cfg := &config.Config{}
	svc := NewBillingCacheService(cache, nil, nil, nil, nil, nil, cfg, nil)
	t.Cleanup(svc.Stop)

	err := svc.CheckBillingEligibility(context.Background(), &User{ID: 1}, &APIKey{ID: 2, Quota: 5}, nil, nil, "")
	require.NoError(t, err)
}
