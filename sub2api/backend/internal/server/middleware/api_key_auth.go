	...
	return func(c *gin.Context) {
		...
		if !skipBilling {
			...
			if subscription != nil {
				...
				// 正额度 API key 的 quota 本身是可扣费的权威额度来源；不能只允许
				// BotSale 生成的 RetailSale key 在用户余额为 0 时通过，否则后台/
				// Web 手工创建的同类 quota key 会被静默 403。
				if !apiKeyHasAuthoritativeQuota(apiKey) && apiKeyBalanceBelowAuthThreshold(apiKey.User.Balance, cfg) {
...
func apiKeyHasAuthoritativeQuota(apiKey *service.APIKey) bool {
	return apiKey != nil && apiKey.BypassesUserBalance()
}
