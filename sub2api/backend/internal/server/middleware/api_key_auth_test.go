...
func TestAPIKeyAuthAllowsRetailSaleWithZeroUserBalance(t *testing.T) {
	gin.SetMode(gin.TestMode)

	user := &service.User{ID: 10, Role: service.RoleUser, Status: service.StatusActive, Balance: 0}
	apiKey := &service.APIKey{
		ID: 105, UserID: user.ID, Key: "retail-sale-zero-balance", Status: service.StatusActive,
		Quota: 2, QuotaUsed: 0, RetailSale: true, User: user,
	}
...
func TestAPIKeyAuthAllowsWebQuotaKeyWithZeroUserBalance(t *testing.T) {
	gin.SetMode(gin.TestMode)

	user := &service.User{ID: 10, Role: service.RoleUser, Status: service.StatusActive, Balance: 0}
	apiKey := &service.APIKey{
		ID: 106, UserID: user.ID, Key: "web-quota-zero-balance", Status: service.StatusActive,
		Quota: 2, QuotaUsed: 0, RetailSale: false, User: user,
	}
	apiKeyRepo := &stubApiKeyRepo{getByKey: func(ctx context.Context, key string) (*service.APIKey, error) {
		if key != apiKey.Key {
			return nil, service.ErrAPIKeyNotFound
