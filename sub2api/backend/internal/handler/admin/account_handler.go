...
const batchUsageFastTimeout = 1500 * time.Millisecond
...
func (h *AccountHandler) GetBatchUsage(c *gin.Context) {
	...
	if !req.Force {
		...
		ctx, cancel = context.WithTimeout(ctx, batchUsageFastTimeout)
		defer cancel()
	}

	usageByAccount, errorsByAccount, err := h.accountUsageService.GetUsageBatch(ctx, accountIDs, req.Force)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
