...
func (r *apiKeyRepository) GetByID(ctx context.Context, id int64) (*service.APIKey, error) {
	...
	out.RetailSale, err = r.isRetailSaleKey(ctx, out.ID)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (r *apiKeyRepository) isRetailSaleKey(ctx context.Context, apiKeyID int64) (bool, error) {
	const query = `
		SELECT EXISTS (
			SELECT 1 FROM api_key_sales_reservation_items WHERE api_key_id = $1
			UNION ALL
			SELECT 1 FROM api_key_sales_reservations
			WHERE fulfilled_api_key_id = $1 AND state = 'completed'
			UNION ALL
			SELECT 1 FROM api_key_sales_fulfillments WHERE api_key_id = $1
		)`
	var retail bool
	if err := scanSingleRow(ctx, r.sql, query, []any{apiKeyID}, &retail); err != nil {
		return false, fmt.Errorf("resolve api key retail sale provenance: %w", err)
	}
	return retail, nil
}
